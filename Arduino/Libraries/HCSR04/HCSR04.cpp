...
void HCSR04::init(uint16_t p_cid, uint16_t p_trigger_pin, uint16_t p_echo_pin, uint32_t p_interval, unsigned int max_cm_distance) {
	if(!inited) {
		cid = p_cid;
		changeInterval(p_interval);
		changePins(p_trigger_pin, p_echo_pin);
		maxDistance = max_cm_distance;
		nextTrigger = millis()+p_interval;
		...
		pinMode(p_echo_pin, INPUT);     // Set echo pin to input (on Teensy 3.x (ARM), pins default to disabled, at least one pinMode() is needed for GPIO mode).
		pinMode(p_trigger_pin, OUTPUT); // Set trigger pin to output (on Teensy 3.x (ARM), pins default to disabled, at least one pinMode() is needed for GPIO mode).
#endif

#if defined (ARDUINO_AVR_YUN)
		pinMode(p_echo_pin, INPUT);     // Set echo pin to input for the Arduino Yun, not sure why it doesn't default this way.
#endif
...
void HCSR04::changePins(uint16_t p_trigger_pin, uint16_t p_echo_pin) {
#if DO_BITWISE == true
	_triggerBit = digitalPinToBitMask(p_trigger_pin); // Get the port register bitmask for the trigger pin.
	_echoBit = digitalPinToBitMask(p_echo_pin);       // Get the port register bitmask for the echo pin.

	_triggerOutput = portOutputRegister(digitalPinToPort(p_trigger_pin)); // Get the output port register for the trigger pin.
	_echoInput = portInputRegister(digitalPinToPort(p_echo_pin));         // Get the input port register for the echo pin.

	_triggerMode = (uint8_t *)portModeRegister(digitalPinToPort(p_trigger_pin)); // Get the port mode register for the trigger pin.
#else
	_triggerPin = p_trigger_pin;
	_echoPin = p_echo_pin;
#endif
}

void HCSR04::changeInterval(uint32_t p_interval) {
	if(p_interval<50UL) {
		interval = 50UL;
	} else {
		interval = p_interval;
...
void HCSR04::updateNextTrigger() {
	nextTrigger = millis()+interval;
}
...
unsigned long HCSR04::ping_cm(unsigned int max_cm_distance) {
	unsigned long echoTime = HCSR04::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
...
unsigned long HCSR04::ping_in(unsigned int max_cm_distance) {
	unsigned long echoTime = HCSR04::ping(max_cm_distance); // Calls the ping method and returns with the ping echo distance in uS.
#if ROUNDING_ENABLED == false
...
unsigned long HCSR04::ping_median(uint8_t it, unsigned int max_cm_distance) {
...
void HCSR04::ping_timer(void (*userFunc)(void)) {
	if (maxDistance > 0) set_max_distance(maxDistance); // Call function to set a new max sensor distance.
...
#endif
		timer_stop();                // Disable timer interrupt
		distance = (int16_t)((micros()-(_max_time-_maxEchoTime)-PING_TIMER_OVERHEAD)/US_ROUNDTRIP_CM); // Calculate ping time including overhead.
...
void HCSR04::timer_us(unsigned int frequency, void (*userFunc)(void)) {
...
void HCSR04::timer_ms(unsigned long frequency, void (*userFunc)(void)) {
	intFunc = HCSR04::timer_ms_cntdwn;  // Timer events are sent here once every ms till user's frequency is reached.
	...
	TIMSK4 = (1<<TOIE4);   // Enable Timer4 interrupt.
#elif defined (__arm__) && defined (TEENSYDUINO)  // Timer for Teensy 3.x
	itimer.begin(HCSR04::timer_ms_cntdwn, 1000); // Set timer to 1ms (1000 uS).
#else
...
void HCSR04::timer_stop() { // Disable timer interrupt.
#if defined (__AVR_ATmega32U4__) // Use Timer4 for ATmega32U4 (Teensy/Leonardo).
	TIMSK4 = 0;
#elif defined (__arm__) && defined (TEENSYDUINO) // Timer for Teensy 3.x
	itimer.end();
...
unsigned int HCSR04::convert_cm(unsigned int echoTime) {
#if ROUNDING_ENABLED == false
...
unsigned int HCSR04::convert_in(unsigned int echoTime) {
#if ROUNDING_ENABLED == false
